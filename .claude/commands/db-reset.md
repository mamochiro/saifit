...
4. Wait for Postgres to be ready: `until docker exec saifit-postgres pg_isready -U saifit; do sleep 1; done`
...
7. Verify:
   ...
   - `SELECT email FROM auth.users WHERE email = 'dev@saifit.local'` returns 1 row
8. Report success or failure with counts.

All commands run from `/Users/sarawut/github/saifit`.
